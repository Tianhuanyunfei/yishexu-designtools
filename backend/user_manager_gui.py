...
class UserManagerGUI:
    ...
    def setup_ui(self):
        ...
        self.tree.column("更新时间", width=150, anchor="center")
    ...
    def load_users(self):
        ...
        try:
            self.cursor.execute("SELECT id, username, created_at, updated_at FROM users ORDER BY id DESC")
    ...
    def search_users(self):
        ...
        try:
            if search_term:
                self.cursor.execute(
                    "SELECT id, username, created_at, updated_at FROM users WHERE username LIKE ? ORDER BY id DESC",
                    (f"%{search_term}%",)
                )
            else:
                self.cursor.execute("SELECT id, username, created_at, updated_at FROM users ORDER BY id DESC")
